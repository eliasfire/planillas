...
<?php if($this->defaultAction !== 'none'): ?>
    public $defaultAction = '<?= $this->defaultAction; ?>';
...
<?php if (in_array($this->validation, array(1, 3))): ?>
        $this->performAjaxValidation($model, '<?= $this->class2id($this->modelClass)?>-form');
<?php endif; ?>
...
<?php if($this->getUseRelatedRecordBehavior() and ($relation[0] == CActiveRecord::BELONGS_TO || $relation[0] == CActiveRecord::MANY_MANY)):?>
                <?= "if (isset(\$_POST['$this->modelClass']['$key'])) \$model->$key = \$_POST['$this->modelClass']['$key'];\n"?>
<?php endif; ?>
<?php endforeach; ?>
